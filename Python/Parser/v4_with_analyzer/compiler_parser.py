...
from FIP import FIP
from CFG import CFG
from LR1Analyzer import LR1Analyzer
...
def extended_parse(text):
    ...
    return ts, fip, atom_lines


def parse(text):
    ts, fip, atom_lines = extended_parse(text)
    return ts, fip


MY_GRAMMAR = CFG.from_file("my_grammar.txt")
MY_ANALYZER = LR1Analyzer.for_cfg(MY_GRAMMAR)


def parse_and_analyze(text, show_analysis=True):
    ts, fip, atom_lines = extended_parse(text)
    tokens = fip_to_tokens(fip)
    is_accepted = MY_ANALYZER.check_sequence(tokens, show_analysis)
    if is_accepted:
        return ts, fip
    else:
        token_fail_index = len(atom_lines) - (len(tokens) - 1)
        line_number = atom_lines[token_fail_index]
        raise Exception("syntax error at line " + str(line_number))
...
def fip_to_tokens(fip):
    return [CODE_TO_TOKEN[elem.atom_code] for elem in fip.get_as_list()]
